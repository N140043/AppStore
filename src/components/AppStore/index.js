...
class AppStore extends Component {
  ...
  changeSerchInput = event => {
    this.setState({searchInput: event.target.value})
  }

  render() {
    const {activeTab} = this.state
    const filterAppsList = this.filterdAppsList()
      ...
      <div className="home">
        ...
        <div className="search-input-container">
          <input
            ...
            placeholder="Search"
            onChange={this.changeSerchInput}
          />
          <img
        ...
        <ul className="app-container">
          {filterAppsList.map(eachApp => (
            <AppItem appDetails={eachApp} key={eachApp.appId} />
          ))}
          {filterAppsList.length === 0 ? <p>No apps Found</p> : null}
        </ul>
      </div>
    )
  }
